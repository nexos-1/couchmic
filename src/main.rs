//! CouchMic: iPad or iPhone microphone as a Windows microphone. The application lives in app.rs
...
// window; closing it killed CouchMic (and the watchdog opened the next one). For --list/--help
...
#[cfg(not(windows))]
fn main() {
    eprintln!("CouchMic runs on Windows only (WASAPI, COM, VB-CABLE).");
    std::process::exit(1);
}
